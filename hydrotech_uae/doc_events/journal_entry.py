...
def cancel_jv(doc, method):
    ...
    if doc.petty_cash_request:
        frappe.db.sql(""" UPDATE `tabPetty Cash Request` SET agent_outstanding_amount=claim_amount, status='Unpaid' WHERE name=%s """, doc.petty_cash_request)
        frappe.db.commit()